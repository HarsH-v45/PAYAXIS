...
authRouter.post("/login", async (req, res) => {
  ...
  const { rows } = await db.query(
    `SELECT u.id,u.password_hash,u.role,u.is_active,e.id AS employee_id,e.full_name,e.email,e.employee_code FROM users u JOIN employees e ON e.user_id=u.id WHERE u.email=$1`,
  ...
  const tokens = issueTokens({ id:user.id, employeeId:user.employee_id, role:user.role, email:user.email, name:user.full_name });
  logAsync({ action:"USER_LOGIN", entityType:"User", entityId:user.id, actorId:user.employee_id, actorRole:user.role, actorIp:req.ip, description:`${user.full_name} logged in` });
  res.json({ success:true, data:{ ...tokens, user:{ id:user.employee_id, name:user.full_name, email:user.email, role:user.role, code:user.employee_code } } });
});
...
authRouter.post("/refresh", async (req, res) => {
  ...
  try {
    ...
    const { rows } = await db.query(`SELECT u.id,u.role,u.is_active,e.id AS employee_id,e.full_name,e.email FROM users u JOIN employees e ON e.user_id=u.id WHERE u.id=$1`, [payload.sub]);
...
empRouter.post("/:id/salary-revision", requirePermission("salary:revise"), async (req, res) => {
  const { annual_ctc, basicPercent=0.40, components, effectiveFrom, reason } = req.body;
  const { rows } = await db.withTransaction(async (client) => {
    await client.query(`UPDATE salary_structures SET is_active=FALSE WHERE employee_id=$1 AND is_active=TRUE`, [req.params.id]);
  const result =   await client.query(
  `INSERT INTO salary_structures 
   (employee_id, annual_ctc, basic_percent, components, effective_from, is_active, revised_by, revision_reason) 
